...
namespace Sudoku.Solving.Manual.Alses.Mslses
{
	...
	public sealed class SdcTechniqueInfo : MslsTechniqueInfo
	{
		...
		/// <inheritdoc/>
		public override string Name => $"{(IsCannibalistic ? "Advanced " : string.Empty)}Sue de Coq";
		...
		{
			string blockCellsStr = new CellCollection(BlockCells.Offsets).ToString();
			string blockDigitsStr = new DigitCollection(BlockMask.GetAllSets()).ToString(null);
			string lineCellsStr = new CellCollection(LineCells.Offsets).ToString();
			string lineDigitsStr = new DigitCollection(LineMask.GetAllSets()).ToString(null);
			string interCellsStr = new CellCollection(IntersectionCells.Offsets).ToString();
