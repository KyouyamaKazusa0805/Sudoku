...
namespace Sudoku.Solving.Manual.Exocets
{
	...
	/// </summary>
	public sealed class JeTechniqueSearcher : ExocetTechniqueSearcher
	{
		/// <inheritdoc/>
		public JeTechniqueSearcher(bool checkAdvanced) : base(checkAdvanced)
		{
		}


		/// <inheritdoc cref="SearchingProperties"/>
		public static TechniqueProperties Properties { get; } = new(94, nameof(TechniqueCode.Je)) { DisplayLevel = 4 };
		...
		public override void GetAll(IList<TechniqueInfo> accumulator, in SudokuGrid grid)
		{
			foreach (var exocet in Patterns)
			{
				...
				short needChecking = (short)(baseCandsMask & temp);
		...
		{
			short candMask = (short)(grid.GetCandidateMask(cell) & ~temp);
			if (grid.GetStatus(cell) == CellStatus.Empty && candMask != 0
				&& (grid.GetCandidateMask(cell) & baseCandsMask) != 0)
			{
			...
			foreach (short mask in GetCombinations(candidatesMask))
			{
				for (int i = 0; i < 2; i++)
				{
					...
					if (count == mask.PopCount() - 1)
					{
						...
						{
							int p = RegionCells[span[i]][j];
							if (grid.GetStatus(p) != CellStatus.Empty || (grid.GetCandidateMask(p) & mask) == 0
