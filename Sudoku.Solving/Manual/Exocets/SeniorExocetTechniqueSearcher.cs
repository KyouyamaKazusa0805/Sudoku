	...
	public sealed class SeniorExocetTechniqueSearcher : ExocetTechniqueSearcher
	{
		...
		public override void GetAll(IBag<TechniqueInfo> accumulator, IReadOnlyGrid grid)
		{
			...
			foreach (var exocet in Exocets)
			{
				...
				if (grid.GetCandidateMask(b1).CountSet() < 2
					|| grid.GetCandidateMask(b2).CountSet() < 2)
				{
					continue;
				}
				...
				foreach (int[] combination in GetCombinationsOfArray(tempTarget.ToArray(), 2))
				{
					...
					short elimDigits = (short)((
						grid.GetCandidateMask(combination[0])
						| grid.GetCandidateMask(combination[1])) & ~baseCandidatesMask);
					if (!CheckCrossline(
					...
					if (_checkAdvanced)
					{
						...
						if (incompatible != 0)
						{
							...
							for (int k = 0; k < 4; k++)
							{
								cands = (short)(incompatible & grid.GetCandidateMask(compatibleCellsPlayground[k]));
								if (cands == 0)
								{
									continue;
								}
