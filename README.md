...
| `PREFER_ZERO_BITS`           | Indicates whether the integer handling prefers zero bits (unset bits). This symbol is defined when the integer contains more unset bits than set bits in order to optimize the performance. This symbol is only used in extension methods named `PopCount` about integers.<br/>表示是否整数处理的时候更偏好未置位的比特（也就是 0 而不是 1 的比特位）。当整数包含的 0 比特位比 1 比特位要多的话，可以考虑使用这个符号，以便优化性能。这个符号只用于有关整数的 `PopCount` 扩展方法。 |
| `COPY_SYNC`                  | Indicates whether the program copies the grid using synchronized mode. During the execution in `async` methods,  [`Clipboard.SetText`](https://docs.microsoft.com/en-us/dotnet/api/system.windows.forms.clipboard.settext?view=netcore-3.1) always throws [`COMException`](https://docs.microsoft.com/en-us/dotnet/api/system.runtime.interopservices.comexception?view=netcore-3.1),  and the `HResult` property is `0x800401D0`.<br/>表示是否程序以同步的方式复制盘面数据。在异步方法在执行的时候，[`Clipboard.SetText`](https://docs.microsoft.com/en-us/dotnet/api/system.windows.forms.clipboard.settext?view=netcore-3.1) 方法一直都会抛出 [`COMException`](https://docs.microsoft.com/en-us/dotnet/api/system.runtime.interopservices.comexception?view=netcore-3.1) 异常，而且结果是`0x800401D0`。 |
