...
这三个写法的区别是，第一个和第二个是一样的判断：因为 `[1, 2, 4]` 按照顺序，判断的都是前三个数据的数值；只是因为有一个有 `..` 记号，所以编译器会认为后面可能有数据，所以长度判定下，第一个只有三个元素，而第二个则至少有三个元素。

这就是为了避免长度不够就去访问元素而抛出异常，C# 会贴心地做一个“预防性判断”。如果 `arr` 没有这么长呢？假设 `arr` 就俩元素，那么判断 `[1, 2, 4, ..]` 就可能产生一个异常。因此，C# 会自动生成一条预判长度语句：`arr.Length >= 3`。
...
| `arr is [1, 2, 4, ..]`       | `arr.Length >= 3 && arr[0] == 1 && arr[1] == 2 && arr[2] == 4` |
| `arr is [1, 2, 4, _, _]`     | `arr.Length == 5 && arr[0] == 1 && arr[1] == 2 && arr[2] == 4` |
| `arr is [1, 2, 4, _, _, ..]` | `arr.Length >= 5 && arr[0] == 1 && arr[1] == 2 && arr[2] == 4` |
...
```csharp
if (arr.Length >= 2 && arr[1..^1] is var slice)
    // ...
```

一定要注意，`1..^1` 的语义是“取` [1]` 到 `[^2]` 的元素，而绝对不是取到 `^1`。因为 C# 的范围表达式是取前不取后的（前闭后开的半开区间）。

> 顺带一说，集合模式最大范围能够支持到 `IEnumerable<T>` 这个泛型接口类型上。目前来说，带有 `Slice` 方法的具体类型都可以使用分片模式进行分片；不过 `IEnumerable<T>` 自己是不支持的，因为它自己是没有 `Slice` 这样的方法的。
