...
public sealed partial class AlignedExclusionStepSearcher : StepSearcher
{
	...
	protected internal override Step? Collect(scoped ref AnalysisContext context)
	{
		...
		for (var size = 3; size <= MaxSearchingSize; size++)
		{
			...
			foreach (ref readonly var cellPair in candidateList.GetSubsets(2))
			{
				...
				foreach (ref readonly var tIndices in tailCells.GetSubsets(size - 2))
				{
					...
					// Create hint.
					var step = new AlignedExclusionStep([.. conclusions], [[.. candidateOffsets]], context.PredefinedOptions, [.. cells], [.. lockedCombinations]);
