...
public sealed partial class BivalueUniversalGraveStepSearcher : StepSearcher
{
	...
	protected internal override Step? Collect(scoped ref AnalysisContext context)
	{
		...
		if (candidatesCount > (emptyCellsCount << 1) + 28) // 20 peer cells + 8 digits in the same cell
		{
	...
	)
	{
		...
		foreach (var house in map.CoveredHouses)
		{
			...
			{
				foreach (ref readonly var cells in otherCellsMap.GetSubsets(size).AsReadOnlySpan())
				{
