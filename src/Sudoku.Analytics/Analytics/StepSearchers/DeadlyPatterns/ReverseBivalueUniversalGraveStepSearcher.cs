...
[StepSearcherRuntimeName("StepSearcherName_ReverseBivalueUniversalGraveStepSearcher")]
public sealed partial class ReverseBivalueUniversalGraveStepSearcher : StepSearcher
{
	...
	/// </summary>
	private static readonly CellMap[] GlobalMaps = [~(CellMap)[], .. from chute in Chutes select chute.Cells];
	...
	protected internal override Step? Collect(scoped ref AnalysisContext context)
	{
		...
		foreach (var digitPair in digits.GetAllSets().GetSubsets(2))
		{
			...
			for (var i = 0; i < globalMapUpperBound; i++)
			{
				...
				{
					foreach (ref readonly var cellsChosen in emptyCells.GetSubsets(incrementStep))
					{
		...
		{
			context.Accumulator.AddRange(Step.RemoveDuplicateItems(accumulator));
		}
	...
	)
	{
		...
		foreach (var house in cellsChosen.SharedHouses)
		{
			...
			foreach (ref readonly var cells in otherEmptyCells.GetSubsets(numbersOfOtherDigits - 1))
			{
	...
	/// and returning <see langword="false"/> for the method <see cref="UniqueLoopStepSearcher.IsValidLoop(List{Cell})"/>
	/// above this.
