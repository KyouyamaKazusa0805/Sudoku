...
public sealed partial class UniqueMatrixStepSearcher : StepSearcher
{
	...
	private UniqueMatrixType1Step? CheckType1(ref readonly Grid grid, ref StepAnalysisContext context, ref readonly CellMap pattern, Mask mask)
	{
		...
		foreach (var digits in mask.GetAllSets().GetSubsets(4))
		{
			...
			var step = new UniqueMatrixType1Step(conclusions.AsReadOnlyMemory(), [[.. candidateOffsets]], context.Options, in pattern, digitsMask, elimCell * 9 + extraDigit);
	...
	private UniqueMatrixType2Step? CheckType2(ref StepAnalysisContext context, ref readonly CellMap pattern, Mask mask)
	{
		...
		foreach (var digits in mask.GetAllSets().GetSubsets(4))
		{
			...
			var step = new UniqueMatrixType2Step(conclusions.AsReadOnlyMemory(), [[.. candidateOffsets]], context.Options, in pattern, digitsMask, extraDigit);
			if (context.OnlyFindOne)
			{
				return step;
			}
