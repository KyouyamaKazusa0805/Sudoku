		...
		foreach (var pivot in EmptyCells)
		{
			...
			foreach (var rbCurrentMap in rbList)
			{
				...
				foreach (var cbCurrentMap in cbList)
				{
					...
					{
						foreach (ref readonly var selectedBlockCells in blockMap.GetSubsets(i).AsReadOnlySpan())
						{
							...
							{
								foreach (ref readonly var selectedRowCells in rowMap.GetSubsets(j).AsReadOnlySpan())
								{
									...
									{
										foreach (ref readonly var selectedColumnCells in columnMap.GetSubsets(k).AsReadOnlySpan())
										{
