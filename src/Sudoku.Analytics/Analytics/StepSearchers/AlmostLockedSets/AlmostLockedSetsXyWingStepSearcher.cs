...
[Obsolete(ObsoletionMessages.StepSearchersAreHiddenOnPurpose, false, UrlFormat = "https://github.com/KyouyamaKazusa0805/Sudoku/issues/735")]
public sealed partial class AlmostLockedSetsXyWingStepSearcher : StepSearcher
{
	...
	/// </summary>
	[SettingItemName(SettingItemNames.AllowCollisionOnAlmostLockedSetsXyWing)]
		...
		for (var (i, length) = (0, alses.Length); i < length - 1; i++)
		{
			...
			for (var j = i + 1; j < length; j++)
			{
				...
				if (mask != 0)
				{
					...
					{
						if ((map & CandidatesMap[digit]).FirstSharedHouse != 32)
						{
