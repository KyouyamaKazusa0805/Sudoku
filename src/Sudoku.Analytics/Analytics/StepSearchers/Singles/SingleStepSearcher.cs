		...
		for (var (i, digit) = (0, context.PreviousSetDigit); i < 9; i++, digit = (digit + 1) % 9)
		{
			...
			for (var house = 0; house < 27; house++)
			{
				...
				var step = new FullHouseStep(
					...
					new[] { View.Empty | new HouseViewNode(WellKnownColorIdentifierKind.Normal, house) },
					resultCell,
					digit
				);
	...
	private Step? CheckForHiddenSingleAndLastDigit(scoped in Grid grid, int digit, int house)
	{
		...
		if (EnableLastDigit)
		{
			...
			for (var i = 0; i < 81; i++)
			{
				...
				{
					digitCount++;
					cellOffsets.Add(new(WellKnownColorIdentifierKind.Normal, i));
				}
			}
		...
		return new HiddenSingleStep(
			...
			new[]
			{
				View.Empty
					...
					| (enableAndIsLastDigit ? null : new HouseViewNode(WellKnownColorIdentifierKind.Normal, house))
			},
			resultCell,
			digit,
			house,
