...
public sealed class CellLine : DrawingElement
{
	...
	/// <summary>
	/// Initializes a <see cref="CellLine"/> instance via the specified details.
	/// </summary>
	...
	/// </param>
	public CellLine(Color strokeColor, double width, double paneSize, double outsideOffset, byte order)
	{
		_paneSize = paneSize;
	...
	public double PaneSize
	{
		...
		set
		{
			var ((x1, y1), (x2, y2)) = PointConversions.GetCellLine(_paneSize = value, _outsideOffset, Order);
			_line.X1 = x1;
			_line.X2 = x2;
			_line.Y1 = y1;
			_line.Y2 = y2;
	...
	public double OutsideOffset
	{
		...
		set
		{
			var ((x1, y1), (x2, y2)) = PointConversions.GetCellLine(_paneSize, _outsideOffset = value, Order);
			_line.X1 = x1;
			_line.X2 = x2;
			_line.Y1 = y1;
			_line.Y2 = y2;
	...
	private string DebuggerDisplayView
	{
		get
		{
			...
			return $"{nameof(CellLine)} {{ Start = {(x1, y1)}, End = {(x2, y2)} }}";
		}
	}
#endif
