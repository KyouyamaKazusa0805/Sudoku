...
internal static class BackdoorSearcher
{
	...
	public static Conclusion[] GetBackdoors(scoped in Grid grid)
	{
		...
		if (CommonLogicalSolvers.SstsOnly.Analyze(grid) is { IsSolved: true })
		{
		...
		foreach (var cell in grid.EmptyCells)
		{
			...
			if (!CommonLogicalSolvers.SstsOnly.Analyze(case1Playground).IsSolved)
			{
				continue;
			}
			...
			foreach (var digit in (short)(grid.GetCandidates(cell) & ~(1 << solution[cell])))
			{
				...
				if (!CommonLogicalSolvers.SstsOnly.Analyze(case2Playground).IsSolved)
				{
					continue;
				}
