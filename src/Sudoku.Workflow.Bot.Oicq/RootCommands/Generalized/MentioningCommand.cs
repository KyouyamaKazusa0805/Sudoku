		...
		switch (context)
		{
			...
			case { DrawingContext: var drawingContext, ExecutingCommand: "开始绘图" }:
			{
				var task = args switch
				{
					...
					["顺钟面", var s] => DrawingOperations.AddClockfaceNodesAsync(messageReceiver, drawingContext, s, true),
					...
					["四邻居", var s] => DrawingOperations.AddNeighborNodesAsync(messageReceiver, drawingContext, s, true),
					...
					["标记", var s, var l] => DrawingOperations.AddCellPencilmarkNodesAsync(messageReceiver, drawingContext, s, l),
					["反标记", var s] => DrawingOperations.RemoveCellPencilmarkNodesAsync(messageReceiver, drawingContext, s),
					["四格", var s, var l] => DrawingOperations.AddQuadHintNodesAsync(messageReceiver, drawingContext, s, l),
					["反四格", var s] => DrawingOperations.RemoveQuadHintNodesAsync(messageReceiver, drawingContext, s),
					["田大", var s, var d] => DrawingOperations.AddQuadMaxNodesAsync(messageReceiver, drawingContext, s, d),
					["反田大", var s] => DrawingOperations.RemoveQuadMaxNodesAsync(messageReceiver, drawingContext, s),
					["三角和", var s, var d] => DrawingOperations.AddTriangleSumNodesAsync(messageReceiver, drawingContext, s, d),
				...
				static T f1<T>(ColorIdentifier i, Cell c) where T : IconViewNode => (T)Activator.CreateInstance(typeof(T), new[] { i, c })!;

				static T f2<T>(Cell c) where T : IconViewNode => (T)Activator.CreateInstance(typeof(T), new[] { default(ColorIdentifier), c })!;
			}
		}
	}
}
