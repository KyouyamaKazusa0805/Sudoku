...
public static class PuzzleLibraryOperations
{
	...
	public static Grid LoadFromCachedPath(string groupId, PuzzleLibrary puzzleLibrary)
	{
		...
		if (Deserialize<PuzzleProgress>(File.ReadAllText(currentCachedPath), DefaultOptions) is not { Puzzle: var result })
		{
			return Grid.Undefined;
		}
	...
	/// <summary>
	/// 往本地缓存一个用户完成的题目数据。这样做是为了保证用户在下回使用机器人回答问题（比如它要抽取新题目的时候），校验题目是否已经完成。
	/// </summary>
	/// <param name="grid">数独题目。</param>
	/// <param name="puzzleLibrary">
	/// <para>用户使用的题库。</para>
	/// <para>
	/// 该参数实际上只用到了其中的 <see cref="PuzzleLibrary.Name"/> 属性，不过有可能以后有改动，
	/// 以后可能会使用额外的属性数值，因此这里我们不使用直接传入 <see cref="string"/> 题库名称的方式。
	/// </para>
	/// </param>
	/// <param name="groupId">题库所在群的群号（QQ）。</param>
	/// <seealso cref="PuzzleLibrary.Name"/>
	[MethodImpl(MethodImplOptions.Synchronized)]
	public static void SaveToCachedPath(string groupId, PuzzleLibrary puzzleLibrary, scoped in Grid grid)
	{
		var folder = Environment.GetFolderPath(SpecialFolder.MyDocuments);
		if (!Directory.Exists(folder))
		{
		...
		var libName = puzzleLibrary.Name;
		var currentCachedPath = $"""{groupLibraryFolder}\{libName}.json""";
		File.WriteAllText(currentCachedPath, Serialize(new PuzzleProgress { Puzzle = grid }, DefaultOptions));
	}
