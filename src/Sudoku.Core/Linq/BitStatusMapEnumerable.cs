...
{
	/// <summary>
	/// Projects each <see cref="Cell"/> of a <see cref="CellMap"/> to a <see cref="Mask"/>, flattens the resulting sequence into one sequence,
	...
	/// <param name="resultSelector">A transform function to apply to each element of the intermediate <see cref="Mask"/>.</param>
	/// <returns>
	/// A <see cref="ReadOnlySpan{T}"/> whose elements are the result of invoking the one-to-many transform function
	/// <paramref name="collectionSelector"/> on each element of <paramref name="this"/> and then mapping each of those sequences
	...
	/// <summary>
	/// Projects each <see cref="Candidate"/> of a <see cref="CandidateMap"/> to a <see cref="Mask"/>,
