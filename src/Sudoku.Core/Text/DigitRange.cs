...
public readonly partial struct DigitRange : IEquatable<DigitRange>, IEqualityOperators<DigitRange, DigitRange, bool>, ISimpleParsable<DigitRange>
{
	...
	[MethodImpl(MethodImplOptions.AggressiveInlining)]
	private DigitRange(Mask digits, ConclusionType type) => (DigitsMask, ConclusionType) = (digits, type);
	...
	public static DigitRange Parse(string str)
	{
		return str switch
		{
			...
			['!', var digit and >= '1' and <= '9'] => new((Mask)(1 << digit - '1'), Elimination),
			['!', '-'] => AllDigitsRemoving,
		...
		static Mask? separateMaskRange(string digits)
		{
			var result = (Mask)0;
