...
public unsafe partial record struct Chain(int Mask) : IValueEquatable<Chain>
{
	...
	/// </summary>
	private Chain[]? _rawParents = null;


	/// <summary>
	/// Initializes a <see cref="Chain"/> instance using the specified cell, digit and the status
	...
	public Chain(byte cell, byte digit, bool isOn) : this(ConstructMask(cell, digit, isOn))
	{
	}

	/// <summary>
	/// Initializes a <see cref="Chain"/> instance using the specified cell, digit,
	...
	[MethodImpl(MethodImplOptions.AggressiveInlining)]
	public Chain(byte cell, byte digit, bool isOn, in Chain parent) : this(ConstructMask(cell, digit, isOn, 1))
	{
		_rawParents = new Chain[7];
	...
	[MethodImpl(MethodImplOptions.AggressiveInlining)]
	internal Chain(byte cell, byte digit, bool isOn, Chain[] parents)
	...
	public readonly int AncestorsCount
	{
		get
		{
			var ancestors = new List<Chain>();
			for (List<Chain> todo = new() { this }, next; todo.Count != 0; todo = next)
			{
				next = new();
	...
	/// </summary>
	public readonly IReadOnlyList<Chain> WholeChain
	{
		get
		{
			List<Chain> todo = new() { this }, tempList = new(), done = new();
			while (todo.Count != 0)
			{
				var next = new List<Chain>();
				foreach (var p in todo)
				{
					bool contains = false;
	...
	/// </exception>
	[MethodImpl(MethodImplOptions.AggressiveInlining)]
	public void AddParent(Chain chain)
	{
		switch (ParentsCount)
		{
			case 0:
