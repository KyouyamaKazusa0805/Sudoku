		...
		context.RegisterSourceOutput(
			context.SyntaxProvider
				.ForAttributeWithMetadataName(
					"System.Diagnostics.CodeGen.DependencyPropertyAttribute`1",
				...
				.Where(static pair => pair is (not null, { AssemblyName: "SudokuStudio" }))
				.Select(static (data, _) => data.Left!.Value)
				.Collect(),
			output
		);
