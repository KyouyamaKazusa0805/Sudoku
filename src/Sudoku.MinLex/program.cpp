...
int mappers_size(void);
void clear_mappers(void);
void mappers_add(Mapper* const);
...
static void clear_mappers(void) {
	int i = 0;
	for (; i < 100; i++) {
		theMaps[i] = NULL;
	}
					...
					for (int toRow = 0; toRow < 9; toRow++) {
						...
						for (int col = 0; col < 9; col++) {
							...
							if (nSet == nGivens) {
								...
								mappers_add(&map); //this will automatically ignore duplicates
							}
						} //col
					} //toRow
				nextColsPerm:
...
static void map(const char* src, char* results) {
	char* r = results;
	int i = 0;
	for (; theMaps[i] != NULL; i++) {
		for (int i = 0; i < 81; i++) { //debug backward mapping, should display the original
			r[i] = theMaps[i]->cell[i] == 99 ? 0 : theMaps[i]->label[(int)src[(int)theMaps[i]->cell[i]]];
		}
		r += 81;
	}
}

static int mappers_size(void) {
	int i = 0;
	for (; theMaps[i] == NULL; i++);
	return i;
}

static void mappers_add(Mapper* const item) {
	theMaps[mappers_size()] = item;
}
