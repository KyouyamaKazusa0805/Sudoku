...
public interface IPointCalculator
{
	...
	public static sealed IPointCalculator CreateConverter(float size, float offset = 10) => new PointCalculator(new(size, size), offset);
}
...
file sealed class PointCalculator : IPointCalculator
{
	...
	public RectangleF GetMouseRectangle(scoped in Candidates map)
		=> (map, CandidateSize) switch
		{
			([var min, .., var max], var (cw, ch)) => (min / 9, min % 9, max / 9, max % 9) switch
			{
				var (cMin, dMin, cMax, dMax) => (GetMousePointInCenter(cMin, dMin), GetMousePointInCenter(cMax, dMax)) switch
				{
					var (a, b) => (a with { X = a.X - cw / 2, Y = a.Y - ch / 2 }, b with { X = b.X + cw / 2, Y = b.Y + ch / 2 }) switch
					{
	...
	public (PointF LeftUp, PointF RightDown) GetAnchorsViaHouse(int house)
		=> house switch
		{
			>= 0 and < 9 => (house % 3, house / 3) switch { var (v1, v2) => (GridPoints[v1 * 9, v2 * 9], GridPoints[v1 * 9 + 9, v2 * 9 + 9]) },
	...
	public PointF GetMousePointInCenter(int cell)
		=> (this, GridPoints[cell % 9 * 3, cell / 9 * 3]) switch { ({ CellSize: var (cw, ch) }, var (x, y)) => new(x + cw / 2, y + ch / 2) };
