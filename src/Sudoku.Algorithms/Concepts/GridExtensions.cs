...
public static class GridSolvingExtensions
{
	...
	public static bool ExactlyValidate(this scoped in Grid @this, out Grid solutionIfValid, [NotNullWhen(true)] out bool? sukaku)
	{
		...
		if (Solver.CheckValidity(@this.ToString(), out var solution))
		{
	...
	private static Grid Unfix(this scoped in Grid @this, scoped in CellMap pattern)
	{
		Argument.ThrowIfFalse(@this.IsSolved, "The current grid must be solved.");
