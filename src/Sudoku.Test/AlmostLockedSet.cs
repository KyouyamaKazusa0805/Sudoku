...
#endif
{
	...
	public static AlmostLockedSet[] Gather(in Grid grid)
	{
		...
		foreach (int cell in grid.BivalueCells)
		{
			result.Add(new(grid.GetCandidates(cell), new() { cell }, PeerMaps[cell] & grid.EmptyCells));
		}

		// Get all non-bi-value-cell ALSes.
		for (int region = 0; region < 27; region++)
		{
			if ((RegionMaps[region] & grid.EmptyCells) is not { Count: >= 3 } tempMap)
			{
				continue;
			}
					...
					result.Add(
						new(
							...
							region < 9 && coveredLine is >= 9 and not Constants.InvalidFirstSet
								? ((RegionMaps[region] | RegionMaps[coveredLine]) & grid.EmptyCells) - map
								: tempMap - map
						)
					);
				}
