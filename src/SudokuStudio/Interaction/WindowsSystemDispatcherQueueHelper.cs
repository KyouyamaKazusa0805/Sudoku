...
/// <summary>
/// Represents a helper type for <see cref="winsys::DispatcherQueue"/>.
/// </summary>
...
internal sealed partial class WindowsSystemDispatcherQueueHelper
{
	...
	/// of a <see cref="winsys::DispatcherQueue"/> to run queued tasks in priority order on the dispatcher queue's thread.
	/// </summary>
	/// <param name="options">
	...
	/// initializes it with the specified COM apartment, and associates a <see cref="DispatcherQueue"/> with that thread.
	...
	/// then a <see cref="winsys::DispatcherQueue"/> is created and associated with the current thread.
	/// An error results if there is already a <see cref="winsys::DispatcherQueue"/> associated with the current thread.
	...
	/// <para>
	/// <b>Important</b><br/>
	/// The <see cref="DispatcherQueueController"/>, and its associated <see cref="winsys::DispatcherQueue"/>, are WinRT objects.
