...
public partial class DataRow : Panel
{
	...
	/// <summary>
	/// Initializes a <see cref="DataRow"/> instance.
	/// </summary>
	public DataRow() => Unloaded += DataRow_Unloaded;
	...
	private Panel? InitializeParentHeaderConnection()
	{
		...
		if (this.FindAscendant<ItemsPresenter>() is ItemsPresenter itemsPresenter)
		{
			...
			panel = itemsPresenter.Header is GridLayout or DataTable
				? itemsPresenter.Header as Panel
				: // 3. Otherwise, try and find the inner thing we want.
				itemsPresenter.FindDescendant<Panel>(static (element) => element is GridLayout or DataTable);
		...
		panel ??= this.FindAscendant<Panel>(static element => element is GridLayout or DataTable);
	...
	protected override Size MeasureOverride(Size availableSize)
	{
		...
		if (Children.Count > 0)
		{
			...
			else if (_parentTable is not null && _parentTable.Children.Count == Children.Count)
			{
				...
				for (var i = 0; i < Children.Count; i++)
				{
					if (_parentTable.Children[i] is DataColumn { CurrentWidth.GridUnitType: GridUnitType.Auto } col)
					{
						...
						if (i == 0 && _isTreeView)
						{
							...
							var parentContainer = this.FindAscendant("MultiSelectGrid") as GridLayout;
							if (parentContainer is not null)
							{
								...
								for (var j = 0; j < parentContainer.Children.Count - 1; j++)
								{
			...
			else if (_parentPanel is GridLayout grid
	...
	protected override Size ArrangeOverride(Size finalSize)
	{
		...
		var spacing = _parentTable?.ColumnSpacing ?? (_parentPanel as GridLayout)?.ColumnSpacing ?? 0D;
		var width = 0D;

		if (_parentPanel is not null)
		{
			var i = 0;
			foreach (var child in from e in Children where e.Visibility == Visibility.Visible select e)
			{
				if (_parentPanel is GridLayout grid && column < grid.ColumnDefinitions.Count)
				{
					width = grid.ColumnDefinitions[column++].ActualWidth;
				}
				// TODO: Need to check Column visibility here as well...
				else if (_parentPanel is DataTable table && column < table.Children.Count)
				{
					// TODO: This is messy...
				...
				child.Arrange(
					i == 0
						? new(x, 0, width, finalSize.Height)
						: new(x - _treePadding, 0, width, finalSize.Height) // If we're in a tree, remove the indentation from the layout of columns beyond the first.
				);
			...
			return new(x - spacing, finalSize.Height);
		}
