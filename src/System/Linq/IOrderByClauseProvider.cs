...
public interface IOrderByClauseProvider<T> : ILinqProvider<T>
{
	...
	sealed unsafe IOrderedEnumerable<T> OrderByUnsafe<TKey>(delegate*<T, TKey> keySelector) => OrderBy(e => keySelector(e));

	/// <inheritdoc cref="OrderByDescending{TKey}(Func{T, TKey})"/>
	sealed unsafe IOrderedEnumerable<T> OrderByDescendingUnsafe<TKey>(delegate*<T, TKey> keySelector)
		=> OrderByDescending(e => keySelector(e));

	/// <inheritdoc cref="ThenBy{TKey}(Func{T, TKey})"/>
	sealed unsafe IOrderedEnumerable<T> ThenByUnsafe<TKey>(delegate*<T, TKey> keySelector) => ThenBy(e => keySelector(e));

	/// <inheritdoc cref="ThenByDescending{TKey}(Func{T, TKey})"/>
	sealed unsafe IOrderedEnumerable<T> ThenByDescendingUnsafe<TKey>(delegate*<T, TKey> keySelector)
