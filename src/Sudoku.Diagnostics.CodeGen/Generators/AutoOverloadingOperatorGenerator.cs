			...
			foreach (var (operatorKinds, typeSymbol, compilation) in data.CastToNotNull())
			{
				...
				string op_True(INamedTypeSymbol typeSymbol)
				{
					...
					var fullName = isLargeStruct
						? $"scoped in {f(typeSymbol)}"
						: typeSymbol.IsRefLikeType ? $"scoped {f(typeSymbol)}" : f(typeSymbol);
					var crefText = isLargeStruct ? $@" cref=""global::System.IBooleanOperators{{TSelf}}.{nameof(op_True)}(TSelf)""" : string.Empty;
				...
				string op_False(INamedTypeSymbol typeSymbol)
				{
					...
					var fullName = isLargeStruct
						? $"scoped in {f(typeSymbol)}"
						: typeSymbol.IsRefLikeType ? $"scoped {f(typeSymbol)}" : f(typeSymbol);
					var crefText = isLargeStruct ? $@" cref=""global::System.IBooleanOperators{{TSelf}}.{nameof(op_False)}(TSelf)""" : string.Empty;
					var executingCode = typeSymbol.MemberNames.Contains("_count")
						? "value._count == 0"
						: typeSymbol.MemberNames.Contains("_length")
							? "value._length == 0"
