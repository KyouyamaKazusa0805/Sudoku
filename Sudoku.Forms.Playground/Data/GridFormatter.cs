		...
		private string ToMultiLineStringCore(Grid grid)
		{
			...
			foreach (var (i, _) in valuesByColumn)
			{
				...
				for (int j = 0; j < 9; j++)
				{
					...
					int comparer = Math.Max(candidatesCount, z);
			...
			for (int i = 0; i < 13; i++)
			{
				switch (i)
				{
					...
					default: // Print values and tabs.
					{
						...
						PrintValueLines(valuesByRow[target], '|', '|', maxLengths, sb);
						break;
					}
				}
			}
			...
			IList<short> valuesByRow, int start, int end, int[] maxLengths, StringBuilder sb)
		{
			...
			for (int i = start; i <= end; i++)
			{
				...
				switch (cellStatus)
				{
					case CellStatus.Modifiable:
					{
						sb.Append($"<{digit + 1}>".PadRight(maxLengths[i]));
						break;
					}
					case CellStatus.Given:
					{
						sb.Append((TreatValueAsGiven ? $"<{digit + 1}>" : $"*{digit + 1}*").PadRight(maxLengths[i]));
						break;
					}
					default:
					{
						sb.Append(PrintCandidates(value).PadRight(maxLengths[i]));
						break;
					}
				}
				sb.Append(i != end ? "  " : " ");
