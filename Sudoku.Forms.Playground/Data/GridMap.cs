	...
	public partial struct GridMap : IEnumerable<bool>, IEquatable<GridMap>
	{
		...
		public GridMap(IEnumerable<int> offsets, InitializeOption initializeOption)
		{
			...
			switch (initializeOption)
			{
				...
				case InitializeOption.ProcessPeersWithoutItself:
				{
					void process(ref long low, ref long high, int peer) =>
		...
		public bool IsNotEmpty => _high != 0 || _low != 0;
		...
		/// </summary>
		public IEnumerable<int> CoveredRegions
		{
			get
			{
				for (int i = 0; i < 27; i++)
		...
		public bool this[int offset]
		{
			...
			get => ((new[] { _low, _high }[offset / Shifting] >> offset % Shifting) & 1) != 0;
			[MethodImpl(MethodImplOptions.AggressiveInlining)]
			set
			{
		...
		public void Deconstruct(out long high, out long low) =>
			(high, low) = (_high, _low);
		...
		/// </returns>
		public bool Equals(GridMap other) =>
		...
		[MethodImpl(MethodImplOptions.AggressiveInlining)]
		public bool Overlaps(GridMap other) => (this & other).IsNotEmpty;
		...
		public bool AllCellCovers(int regionOffset) =>
		...
		/// </param>
		public bool AllSetsAreInOneRegion(out int? region)
		{
			for (int i = 0; i < 27; i++)
			{
				if ((_high & ~CoverTable[i, 0]) == 0 && (_low & ~CoverTable[i, 1]) == 0)
